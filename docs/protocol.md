...
При Windows-входе с собственным `deviceId` сервер создал отдельную активную сессию, но в приложении запись устройства отображалась без нормального имени/модели. Значит, авторизация и device metadata — разные части протокола.

Нужно отдельно определить endpoint, который приложение использует для отправки/обновления `brand`, `model`, версии ОС и/или имени устройства. До этого `deviceId` уже существующей Windows-сессии менять не следует.
