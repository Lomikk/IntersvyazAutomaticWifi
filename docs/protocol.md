...
В наблюдавшемся HAR у `stepOne/stepTwo/stepThree` не было обязательных cookies, CSRF-токена или отдельного transaction ID. Прямой `POST /stepTwo` без промежуточного `GET /stepTwo` выделен как отдельный эксперимент; если сервер примет его с ожидаемым `302 -> stepThree`, промежуточные `GET /stepTwo` и `GET /stepThree` не нужны рабочему клиенту.
