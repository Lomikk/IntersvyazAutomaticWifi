...
Проводились несколько диагностических измерений. В ранних latency-тестах `/stepTwo` намеренно не вызывался.
...
### 2.5. Рабочий polling-профиль

На основе измерений для MVP зафиксировано следующее расписание. Все значения — **абсолютные моменты от запуска `/stepOne`**, а не паузы после завершения предыдущего GET:
...
5. Для production-клиента свежий Wi‑Fi-код с `id > baselineId` считается достаточным доказательством, что backend обработал `/stepOne`: `/stepTwo` отправляется сразу после получения кода, без ожидания browser-facing `302`. Это осознанная latency-оптимизация поверх наблюдавшегося протокола; исторический end-to-end эксперимент ниже всё ещё фиксирует, что в том запуске `302` был получен до фактической отправки `/stepTwo`.

В успешном end-to-end запуске первый GET стартовал примерно в `100.2 ms` и уже вернул свежий код; код был обработан клиентом примерно в `178.4 ms`. Второй GET успел стартовать в `150.2 ms`, потому что расписание не зависит от завершения предыдущего запроса. `/stepOne` был замечен завершённым примерно в `207.4 ms`.
...
Реализации: `experiments/07-fast-code-acquisition.ps1` и `experiments/08-direct-step-two.ps1`.
...
На captive-сети ранее была подтверждена доступность `api.is74.ru` через walled garden при обходе проблемного DNS. Это учитывается в отдельной стратегии IP-кэша ниже.

## 3. Captive portal

### Production transport policy (HTTPS)

Встроенный C#-клиент отправляет **оба запроса с чувствительными данными** —
`POST https://w.is74.ru/stepOne` (телефон) и прямой
`POST https://w.is74.ru/stepTwo` (телефон и свежий Wi-Fi-код) — исключительно
по HTTPS. `GET /stepTwo` и `GET /stepThree` по-прежнему не требуются.
Наблюдаемая в ответе `Location: http://w.is74.ru/stepTwo?...` допускается
как свидетельство этапа протокола, **но не как адрес отправки кода**:
клиент строит адрес прямого POST самостоятельно на ожидаемом HTTPS host.
Автоматического fallback на HTTP при ошибке DNS/TLS нет. Проверка
имени и цепочки сертификата не отключается. HTTPS end-to-end ранее
подтверждён в одном полевом тесте, совместимость со всеми вариантами captive-
состояния ещё требует наблюдения.

Ниже в этом разделе сохранены **исторические HTTP URL** из HAR и
экспериментальных замеров; они описывают поведение официальной страницы,
а не рекомендуемый транспорт production-клиента. Отдельная проверка
`GET http://online.susu.ru/` намеренно остаётся HTTP: она служит для
определения captive-состояния и не содержит номера телефона/кода.
...
Этот GET нужен браузеру для показа формы, но **не нужен автоматическому клиенту**. Это подтверждено отдельным end-to-end тестом: прямой `POST /stepTwo` без предварительного `GET /stepTwo` был принят сервером.
...
Отдельный end-to-end эксперимент подтвердил, что этот POST можно выполнить **напрямую после получения кода**, не открывая страницу `/stepTwo`. В успешном запуске прямой POST начался примерно в `220.5 ms`, завершился примерно в `672.1 ms`, занял около `451.6 ms` RTT и вернул `HTTP 302 → stepThree`.

`stepThree` является достаточным success-marker в заголовке `Location`; рабочему клиенту **не требуется выполнять `GET /stepThree`**.

В наблюдавшемся HAR и в успешном прямом тесте не потребовались отдельные CSRF-поля или transaction ID. Для консервативной реализации `/stepOne` и `/stepTwo` всё равно удобно выполнять одним portal `HttpClient`, сохраняя естественное состояние HTTP-клиента.

### 3.4. Подтверждённый минимальный captive-протокол

Успешно проверена следующая последовательность:
...
После `302 → stepThree` Интернет был подтверждён отдельной connectivity-проверкой. Первый успешный connectivity-check в конкретном запуске завершился примерно в `2.87 s` от старта `/stepOne`; это только верхняя наблюдаемая граница, а не точный момент снятия captive-ограничения, потому что сама проверка имела ненулевой RTT.

Таким образом, браузерные переходы через страницы `stepTwo` и `stepThree` исключены из целевого автоматического протокола.

Полный обезличенный отчёт эксперимента: `docs/experiments/08-direct-step-two.md`.
...
## 6. DNS, direct-IP и connectivity probe в captive-сети

### Production DNS policy

Ранее рассматривался дизайн `cached-IP-first`: сначала физически подключаться к сохранённому IP, сохраняя исходный hostname для HTTP/TLS, а обычный DNS использовать только как fallback. Эта идея была реализована в C# как эксперимент и затем проверена в реальном captive-состоянии на Windows.

Полевой результат не оправдал её как production default:

- обычный DNS для `api.is74.ru` и `w.is74.ru` занимал порядка десятков миллисекунд;
- прямое соединение к актуальному IP действительно снимало примерно несколько десятков миллисекунд DNS;
- однако один заведомо устаревший cached IP с connect budget `700 ms` дал примерно `700 ms` скрытой задержки **до** перехода к обычному DNS;
- jitter самого `w.is74.ru` местами был значительно больше выигрыша от обхода DNS.

Поэтому production C# использует обычный системный DNS и обычные hostname-based соединения. `CachedDnsConnector` оставлен только как экспериментальная/диагностическая реализация; штатные `HttpClient` приложения его не используют, а фоновый DNS warmup удалён из агента и регистрации.

Для HTTPS API это также сохраняет стандартное поведение hostname/SNI и проверки имени сертификата без собственного транспортного обхода. Проверка отзыва сертификата на captive path остаётся отключённой отдельно (`X509RevocationMode.NoCheck`), потому что Windows Schannel в captive-сети воспроизводимо может получать `CRYPT_E_REVOCATION_OFFLINE`; это **не** отключает проверку имени/цепочки сертификата.

### Локальный connectivity probe

Вместо Microsoft Connect Test выбран локальный endpoint ЮУрГУ:

```text
http://online.susu.ru/
```

Он проверен в двух состояниях на Windows.

При открытом Интернете наблюдается быстрый HTTP redirect:

```http
HTTP/1.1 302 ...
Location: https://online.susu.ru/
```

В тёплом состоянии измерялось около `10 ms` total; первый холодный HTTP-запрос был порядка нескольких десятков миллисекунд.

В реальном captive-состоянии Интерсвязь перехватывает HTTP и возвращает не redirect ЮУрГУ, а небольшой HTML/JS-ответ:

```http
HTTP/1.1 200 OK
...

<html><body>Redirect...<script>window.location.href='http://w.is74.ru/?backuri=online.susu.ru/';</script></body></html>
```

Поэтому production-критерий строгий и не требует чтения body:

```text
GET http://online.susu.ru/
AllowAutoRedirect = false
ResponseHeadersRead

Online только если:
  status is 3xx
  AND Location == https://online.susu.ru/

любой другой ответ:
  Internet не подтверждён / captive
```

Не следует привязываться только к одному конкретному redirect-коду (`302`): допустим обычный `3xx`, но целевой `Location` должен точно нормализоваться в `https://online.susu.ru/`. Это сохраняет строгую семантику probe и не ломается при безвредной смене `302` на `301/308`.

Microsoft Connect Test больше не является основным timing-critical oracle. На той же captive-сети он тоже перехватывался Интерсвязью и возвращал `HTTP 200` с injected HTML вместо строки `Microsoft Connect Test`, то есть функционально различал состояния, но добавлял ненужную внешнюю зависимость.
