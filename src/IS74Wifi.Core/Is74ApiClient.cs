        ...
        CancellationToken cancellationToken = default)
    {
        ...
        using (document)
        {
                ...
                addresses.ValueKind != JsonValueKind.Null)
            {
                ...
                addressesCount = addresses.GetArrayLength();
            }

            if (addressesCount > 0)
            {
                return Is74ApiResult<ConfirmationChecked>.Fail(new Is74ApiFailure(
                    Is74ApiFailureKind.MultipleAddresses,
                    operation));
            }

            return Is74ApiResult<ConfirmationChecked>.Success(new ConfirmationChecked(authId, addressesCount));
        }
    }
