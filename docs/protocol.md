...
## 6. DNS и IP-кэш в captive-сети

DNS не должен быть единственной точкой отказа и не обязан находиться в критическом пути авторизации. Ранее на captive-сети было подтверждено, что API остаётся достижим через walled garden, если соединение направить на известный IP вручную, сохранив hostname `api.is74.ru` для HTTPS.

Для MVP принят следующий дизайн:

1. Пока обычный Интернет доступен, периодически разрешать `api.is74.ru` и `w.is74.ru` обычным DNS и сохранять полученные IP вместе со временем обновления.
2. В captive-сети разрешать клиенту использовать сохранённый IP как **транспортный адрес** без обязательного DNS lookup.
3. Логическое имя хоста не заменять IP-адресом в протоколе. Для `api.is74.ru` URL остаётся `https://api.is74.ru/...`, а TLS должен использовать hostname `api.is74.ru` для SNI и проверки сертификата; фактическое TCP-соединение при этом может идти к закэшированному IP.
4. Для `w.is74.ru` также сохранять `Host: w.is74.ru`, даже если TCP-соединение направлено на сохранённый IP. Это защищает от проблем с virtual-host routing.
5. Если сохранённый IP перестал отвечать, использовать обычный DNS как fallback и при успехе обновить кэш. IP нельзя считать постоянным или навечно хардкодить в программу.

Ожидаемый эффект прежде всего в стабильности и предсказуемости, а не в гарантированном большом ускорении. Нормальный DNS lookup может занимать всего несколько миллисекунд, но captive-DNS способен отвечать медленно, ошибочно или не отвечать вовсе. Удаление DNS из обязательного пути исключает такую случайную задержку.

Для `api.is74.ru` baseline-запрос выполняется до `/stepOne`, поэтому он одновременно прогревает DNS/TCP/TLS/HTTP-соединение к API. Последующие `/pushmessages` GET обычно используют уже готовое соединение, поэтому IP-кэш там важнее как отказоустойчивый fallback. Наибольший потенциальный выигрыш по задержке ожидается у первого запроса к `w.is74.ru` (`POST /stepOne`), потому что именно он находится в `T=0` критического пути.

Таким образом, целевая модель выглядит так:

```text
hostname = идентичность сервиса / Host / TLS SNI
cached IP = физический адрес соединения
DNS       = механизм обновления кэша и fallback, а не обязательный шаг каждого captive-login
```
